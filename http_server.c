...
// Optional: Limit threads (join old ones if needed)
...
struct stat st;
if (stat(full_path, &st) != 0 || !S_ISREG(http://st.st_mode)) {
...
// Read file content
char *body = malloc(http://st.st_size);

if (read(fd, body, http://st.st_size) != http://st.st_size) {
free(body);
close(fd);
send_response(client->sockfd, 500, "text/plain", "Internal Server Error", 21);
goto cleanup;
...
send_response(client->sockfd, 200, mime, body, http://st.st_size);
free(body);
} else if (strcmp(method, "POST") == 0) {
// Example: Echo back body (for demo; extend for real POST handling)
...
*headers = strtok(NULL, "");  // Rest are headers + body
...
// Trim whitespace (not used here but handy)
void trim(char *str) {
